...
public class BoardService {
    ...
    BoardService(BoardProjectApplication boardProjectApplication) {
	...
	// 3. 게시글 작성기능 (파일 업로드 포함/ 로컬 저장)
	// 파일 업로드 기능을 구현할 때, 하나의 entity 로 처리 x
	// 요청정보를 담는 별도의 DTO 클래스를 만들어서 처리
	// entity 는 DB와 연동되는 모델 유지
	// entity 구조의 노출 위험
	// 파일 저장 정책(NCP Object Storage, AWS S3)이 변경되었을 때 entity 까지 영향
...
//			
//		}
	// 3. 게시글 작성기능 (파일 업로드 /NCP Object Storage)
	public void register(BoardRequestDTO dto) throws Exception {
		// 1. NCP Object Storage 에 저장 로직 구현
		String savedPath = null; // 업로드 후 저장될 경로
		MultipartFile file = dto.getB_file(); // 요청 DTO에 담긴 파일 가져오기
		if(file != null && !file.isEmpty()) {
			// 1. 파일명 충돌 방지를 위한 로직 구현
	...
	public ResponseEntity<Resource> download(Long b_idx) throws Exception {
		...
		if(file_path == null || file_path.isBlank()) {
			// 파일 경로변수가 null 이거나 비워져있는 경우 404 오류 보내기
			return ResponseEntity.notFound().build();
		}
		System.out.println(file_path);
		
		//URL에서 파일명 추출
		//URL구조 : NCP Object Storage 의 endPoint + "/" + bucketName + "/" + fileName(key)
		...
		// -> 응답을 브라우저가 어떻게 처리할 지 알려주는 설명서
		// attachment -> 무조건 다운로드
		// filename -> 다운로드할 파일명
		ContentDisposition cd = ContentDisposition.attachment().filename(filename, StandardCharsets.UTF_8)
												.build();
